...
class PascalDataLayerMP(caffe.Layer):
    """
    This is a simple data layer with multi-process prefetch for more efficient data streaming
    """
    def setup(self, bottom, top):
        ...
        # create a batch loader to load the images, this loader will use another process
        self._blob_queue = Queue(10) # queue buffer contains 10 batches at most
        self._prefetch_process = BatchLoader(self._blob_queue, params)
    ...
    def forward(self, bottom, top):
        ...
        Load data
        """
        im_blob, cues_blob, sal_blob = self._blob_queue.get()
        # print("queue size = {}".format(self._blob_queue.qsize()))
        top[0].data[...] = im_blob
        top[1].data[...] = cues_blob
        top[2].data[...] = sal_blob
...
class BatchLoader(Process):
    """Experimental class for prefetching blobs in a seperate process"""
    ...
    def run(self):
        print("BatchLoader started")
        while True:
            tmp_ims, tmp_cues, tmp_sals = [], [], []
            for k in range(self.batch_size):
                im, cue, sal = self._load_next_image()
                tmp_ims.append(im)
                tmp_cues.append(cue)
                tmp_sals.append(sal)
            im_blob = np.stack(tmp_ims, axis=0)
            cue_blob = np.stack(tmp_cues, axis=0)
            sal_blob = np.stack(tmp_sals, axis=0)

            self.queue.put((im_blob, cue_blob, sal_blob))
            # print("put {}".format(self._cur))
