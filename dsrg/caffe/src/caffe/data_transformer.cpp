...
void DataTransformer<Dtype>::TransformImgAndSeg(
  ...
  ///////////////////////////////////// Yao Qi, add min scale, ensure min size no less than min_scale //////////////////////////
  if(param_.has_min_scale()) {
    int min_size = img_height > img_width ? img_width : img_height;
    if(min_size < param_.min_scale()) {
      if(img_height > img_width) {
        float ratio = param_.min_scale() / img_width;
        img_height *= ratio;
        img_width = param_.min_scale();
      }
      else{
        float ratio = param_.min_scale() / img_height;
        img_height = param_.min_scale();
        img_width *= ratio;
      }
      cv::resize(cv_cropped_img, cv_cropped_img, cv::Size(img_width, img_height), 0, 0, cv::INTER_LINEAR);
      cv::resize(cv_cropped_seg, cv_cropped_seg, cv::Size(img_width, img_height), 0, 0, cv::INTER_NEAREST);
    }
  }
  ///////////////////////////////////// Yao Qi, add min scale, ensure min size no less than min_scale //////////////////////////

  ///////////////////////////////////////// Yao Qi, add color jittering //////////////////////////
  if(param_.has_distort_param()) {
    cv_cropped_img = ApplyDistort(cv_cropped_img, param_.distort_param());
  }
  ///////////////////////////////////////// Yao Qi, add color jittering //////////////////////////
